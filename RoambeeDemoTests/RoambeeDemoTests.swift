...
/*
 ...
 class TUNESDemoTests: XCTestCase {
     ...
     func testHeaderView() {
         if listController.viewModel?.getsectionCount() ?? 0 > 0 {
             let headerViewName = listController.viewModel?.getSectionAt(0)
     ...
     func testlistCollectionViewForGRIDCellCreateCellsWithReuseIdentifier() {
         ...
         let list = listController.gridView
         
         let kindString = UICollectionView.elementKindSectionHeader
         let header = list?.dataSource?.collectionView?(list ?? UICollectionView(), viewForSupplementaryElementOfKind:kindString , at: indexPath) as? HeaderView
     
         let section = listController.viewModel?.getSectionAt(0)
         
         XCTAssertEqual(header?.titleLabel.text, section?.sectionName)
         let cell = list?.dataSource?.collectionView(list ?? UICollectionView(), cellForItemAt: indexPath)
     ...
     func test_cell() {
         ...
         //        XCTAssertNotNil(tagCell.tagNameLabel?.text)
